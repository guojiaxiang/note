...
- splice(n,m,x)
    作用：数组截取/克隆/替换
    参数：n：索引，m：个数，x：插入的内容
    返回值：截取后的数组
    原数组：改变
    ```
    var ary=[1,2,8,9,4,5,2,1,4]
    var a=ary.splice()
    console.log(a,ary)=>a=[],ary=[1,2,8,9,4,5,2,1,4]
    //不传参的时候，原数组不变
    var b=ary.splice(0)
    console.log(b,ary)=>b=[1,2,8,9,4,5,2,1,4] ,ary=[]
    //参数为0则表示克隆数组到新数组，原数组清空

    var c=ary.splice(3)
    console.log(c,ary)=>c=[9, 4, 5, 2, 1, 4],ary=[1, 2, 8]
    //只传一个参数的时候，表示从这个索引开始【包括这一项】一直截取到最后，原数组为这个索引之前的数据

    var d=ary.splice(4,2)
    console.log(d,ary) =>d=[4, 5],ary=[1, 2, 8, 9, 2, 1, 4]
    //从索引为n的那一项开始一共截取m项

    var e=ary.splice(4,2,"A")
    console.log(e,ary)=>e=[4, 5],ary=[1, 2, 8, 9, "A", 2, 1, 4]
    //从索引为n的那一项开始一共截取m项,用x代替截取出来的部分

    var f=ary.splice(4,0,"A")
    console.log(f,ary)=>f=[],ary=[1, 2, 8, 9, "A", 4, 5, 2, 1, 4]
    //从索引为n的那一项开始一共截取0项,用x代替截取出来的部分

    var g=ary.splice(ary.length,0,"A")
    console.log(g,ary)=>g=[],ary=[1, 2, 8, 9, 4, 5, 2, 1, 4, "A"]
    //从索引为ary.length的那一项开始一共截取0项,用x代替截取出来的部分
    ```
- reverse
    作用：数组翻转
    参数：无
    ...
    var ary=[1,2,8,9,4,5,2,1,4];
    var a = ary.reverse();
    console.log(a)=>[4, 1, 2, 5, 4, 9, 8, 2, 1]
    console.log(ary)=>[4, 1, 2, 5, 4, 9, 8, 2, 1]
    ```
- sort()
    作用：数组排序，按从小到大
    参数：函数（a，b）
    ...
    原数组：改变
    ```
    var ary=[1,2,-20,8,-1,9,4,55,2,-10,1,4];
    var a=ary.sort()
    console.log(a)=>[-1, -10, -20, 1, 1, 2, 2, 4, 4, 55, 8, 9]
    //不传参数的时候，只能排10以内的数字,有负数的时候,负数是从大到小
    var b=ary.sort(function(a,b){return a-b})
    console.log(b)=>[-20, -10, -1, 1, 1, 2, 2, 4, 4, 8, 9, 55]
    //从小到大
    var c=ary.sort(function(a,b){return b-a})
    console.log(c)=>[55, 9, 8, 4, 4, 2, 2, 1, 1, -1, -10, -20]
    //从大到小
    ```
> 原数组不发生变化的方法
...
- slice(n,m)
    ...
    var d=ary.slice(4,6)
    var e=ary.slice(-5,-3)
    console.log(a)=>a=[1, 2, 8, 9, 4, 5, 2, 1, 4]
    //表示克隆数组
    console.log(b)=>b=[1, 2, 8, 9, 4, 5, 2, 1, 4]
    //表示克隆数组
    console.log(c)=>c=[9, 4, 5, 2, 1, 4]
    //只传一个参数的时候，表示从对应索引开始一直截取到最后
    console.log(d)=>d=[4, 5]
    //传两个参数的时候，表示从索引n开始截取到索引m处，包括n，但不包括m
    console.log(e)=>e=[4,5]
    //如果索引为负数，表示负数+数组长度之和，然后再进行截取，依然遵循包前不包后
    ```
- concat
    作用：给数组的末尾进行拼接
    参数：任何数组类型
    ...
    ```
    var ary=[1,2,8,9,4,5,2,1,4];
    var a=ary.concat(1,"f",true,function(){},[9,9,9,9],{G:8})
    console.log(a)=>a=[1, 2, 8, 9, 4, 5, 2, 1, 4, 1, "f", true, ƒ, 9, 9, 9, 9, {G:8}]
    //可以拼接任何数据类型，如果传入的是数组，会将数组展开然后进行拼接，但是对象数据不能展开
    ```
- indexOf
    作用：根据字符找索引，从前往后看
    参数：指定字符
    返回值：对应的索引。如果找不到则返回-1
    原数组：不改变
    ```
    var ary=[1,2,8,9,4,55,2,1,4];
    var a=ary.indexOf(4)
    var b=ary.indexOf(0)
    var c=ary.indexOf(5)
    console.log(a)=>4
